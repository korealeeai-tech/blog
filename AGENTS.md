...
Avoid creating confusion with inaccurate statements. For dates, versions, prices, policies, product behavior, release status, and other changeable facts, include the checked date or checked version only when timing materially affects the claim. Prefer placing checked dates near the source, caption, or final verification note instead of forcing them into the opening paragraph. Separate facts, interpretations, assumptions, and personal opinions. If something was not verified, say so or leave it out.
