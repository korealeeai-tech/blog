...
이 글에는 실제 Graph 화면을 일부 사용한다. 다만 공개 목적은 내부 기억을 보여주는 것이 아니라, "관계로 연결된 기준"이라는 작동 방식을 설명하는 데 있다. 캡처에서 구체적인 노드명, 원문 경로, 세부 패널을 가린 이유도 그 때문이다.
...
<figure>
	...
	<figcaption>실제 Graph 화면은 구조를 보여주되, 내부 지식 이름과 원문 경로가 노출될 수 있는 영역은 블러 처리했다. 이 이미지는 구조 설명용이지 내부 데이터 공개용이 아니다.</figcaption>
</figure>
...
그래서 실제 화면을 보여줄 때도 전체를 그대로 올리면 안 된다. 이 글의 캡처처럼 구조를 설명하는 데 필요한 중앙 Graph만 남기고, 구체적 노드명과 원문 패널은 블러 처리하는 편이 안전하다. blur만으로 글자가 유추될 수 있으면 mosaic이나 redaction을 함께 써야 한다.
